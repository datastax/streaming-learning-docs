...
The Google BigQuery sink connector reads messages from {pulsar-short} topics and writes them to BigQuery systems.
...
[#configs]
=== Kafka Connect Adapter properties (`configs`)

The properties in the `configs` section define how data is streamed from {pulsar-short} to BigQuery.
...
| no
| `{}`
| A key/value map of config properties to pass to the Kafka connector. See <<kafkaConnectorConfigProperties>>.
...
Set these properties in the `kafkaConnectorConfigProperties` section of the connector configuration.
