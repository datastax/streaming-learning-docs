...
The `cloud.secureConnectBundle` can either be a path to your database's Secure Connect Bundle (SCB) zip or a base64 encoding of the zip provided it in the format `base64:<b64 string>`.
...
These values are provided in the `auth`` area in the preceding {cass-short} connection parameters.
...
These values are provided in the `topic` area in the preceding {cass-short} connection parameters.
