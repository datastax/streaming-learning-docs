...
+
--
// tag::stream-rest-env-vars[]
You need an {product} application token for REST API authentication. This is different from your other {astra-db} application tokens:

. In the {link-astra-portal}, go to *Streaming*, click your tenant's name, and then click the *Settings* tab.
...
. Create environment variables for your application token and web service URL:
+
[source,shell,subs="attributes+"]
----
...
+
--
include::pulsar:connectors/index.adoc[tag=stream-rest-env-vars]
