...
To connect Astra to Decodable, you need some information from your {product} tenant. From the Astra portal home, navigate to the webstore-clicks streaming tenant.
...
. Just as previously done, locate the {product} connection and use the Tenant Details from Astra to fill in the details about the connection as follows. Note the change in topic name.
+
[cols="1,4a",frame=none]
|===
|Connection Type
