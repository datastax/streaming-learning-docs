...
//Compare all with Rabbit
[tabs]
======
Astra Streaming::
+
--
These steps explain how to enable the {starlight-kafka} protocol handler in an Astra Streaming {pulsar-short} cluster, and then connect it to your Kafka instance:
...
[TIP]
====
In Astra Streaming, if you click icon:download[name="Download"] or icon:clipboard[name="Copy"], then a token is included in the `password` field automatically.
