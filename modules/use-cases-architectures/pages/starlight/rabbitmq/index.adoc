...
:description: Learn how to get started using the Starlight for RabbitMQ extension with {pulsar-short} and get hands on by publishing and consuming messages from a topic.

Starlight for RabbitMQ acts as a proxy between your https://www.rabbitmq.com/[RabbitMQ] application and https://pulsar.apache.org/[{pulsar-reg}] cluster.
...
Before you can use a RabbitMQ client to interact with your {pulsar-short} cluster, you need the Starlight for RabbitMQ protocol handler installed in the cluster.
...
+
--
The Starlight for RabbitMQ extension is included in the `luna-streaming-all` image used to deploy a Luna cluster. The Luna helm chart makes deploying the Kafka extension quite easy. Follow the "xref:luna-streaming:components:starlight-for-rabbitmq.adoc[]" guide to create a simple {pulsar-short} cluster with the Starlight for RabbitMQ extension ready for use.
--
Self Managed::
+
--
Already have your own {pulsar-short} Cluster? Or maybe you're using a standalone cluster? Starlight for RabbitMQ can easily be a part of that cluster! Follow the "xref:starlight-for-rabbitmq:installation:getting-started.adoc[]" guide.
--
====
